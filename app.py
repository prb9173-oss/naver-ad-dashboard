...
import requests
import pandas as pd
import json  # 계정 저장을 위한 JSON 라이브러리
import os  # 파일 존재 여부 조회를 위한 OS 라이브러리

# ==========================================
# [데이터 영구 저장] accounts.json 연동 모듈 (가상계정 A, B 제거 반영)
# ==========================================
ACCOUNTS_FILE = "accounts.json"

def load_accounts():
    # 💡 [피드백 반영] 가상 계정 A와 B를 제거하고 깨끗한 빈 상태({})를 기본값으로 잡습니다.
    default_accounts = {}
    
    # 로컬에 저장 폴더/파일이 존재한다면 정상 분석하여 사전을 불러옵니다.
...
# 앱 구동 시 파일로부터 데이터를 불러와 세션에 유지시킵니다.
...
# 계정이 존재하는지와 비어있는 상태에 따른 시각 분기를 처리합니다.
if available_accounts:
    ...
    # 💡 [피드백 반영] 현재 선택된 계정을 파일 및 세션에서 제거하는 삭제 기능 제공
    if st.sidebar.button("🗑️ 선택된 광고 ID 삭제"):
        ...
        st.sidebar.success(f"'{selected_profile}' 계정이 삭제되었습니다.")
        time.sleep(0.5)
        st.rerun()
else:
...
# 계정 선택 시 인풋 박스에 자동으로 데이터를 연동합니다.
...
if st.sidebar.button("💾 위 정보로 광고 ID 등록"):
    if reg_name and input_customer_id and input_api_key and input_secret_key:
        ...
        save_accounts(st.session_state['ad_accounts'])
        st.sidebar.success(f"'{reg_name}' 계정이 성공적으로 저장되었습니다!")
        time.sleep(0.5)
        st.rerun()
    else:
...
# ⚠️ 등록된 계정이 완전히 비어있다면 먼저 등록하도록 메인 화면에서 통제합니다.
if not available_accounts:
    st.info("👈 왼쪽 사이드바의 3번 항목에서 새로운 광고 ID(계정)를 먼저 등록해 주셔야 원활한 조회가 시작됩니다.")
    st.stop()

# 계정 키 안에 'mock' 단어가 포함되어 있거나 비어 있다면 가상 모드 작동
...
# 💡 [피드백 적용] 실제 플레이스 노출 입찰가 가이드 기준을 1,460원으로 정확히 통일 고정합니다.
if selected_ad_type == '플레이스광고':
    avg_bid_val = 1460 # 요청해주신 1,460원 정확한 고정값 연동
        ...
        if raw_df is not None and not raw_df.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "평균 CPC": st.column_config.NumberColumn(alignment="center", format="%,d"), # 원 표시 제외
                    "총비용": st.column_config.NumberColumn(alignment="center", format="%,d"), # 원 표시 제외
                }
            )
